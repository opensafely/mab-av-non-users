...
for(i in seq_along(trt_grp)) {
  ...
  saveRDS(psModel, here("output", "data_models",
                        paste0(trt_grp[i], "_", adjustment_set, "_psModelFit_organ_new.rds")))
  ...
  ggsave(overlapPlot, 
         filename = 
           here("output", "figs", 
                paste0(trt_grp[i], "_", adjustment_set, "_overlap_plot_day5_before_restriction_organ_new.png")),
  ...
  ggsave(overlapPlot2, 
         filename = 
           here("output", "figs", 
                paste0(trt_grp[i], "_", adjustment_set, "_overlap_plot_day5_after_restriction_organ_new.png")),
         width=20, height=14, units="cm")
  ...
  for (j in seq_along(outcomes)){
    ...
    if (is.null(model_PSw()$error)){
      ...
      ggsave(plot, 
             filename = 
               here("output", "figs", 
                    paste0(trt_grp[i], "_",  outcomes[j], "_", adjustment_set, "_cumInc_day5_organ_new.png")),
             width=20, height=14, units="cm")
...
write_csv(estimates,
          here("output", 
               "tables", 
               paste0("cox_models_", data_label, "_", adjustment_set,"_organ_new.csv")))
write_rds(estimates, 
          here("output", 
               "tables", 
               paste0("cox_models_", data_label, "_", adjustment_set, "_organ_new.rds")))
write_csv(log,
          here("output", 
               "tables", 
               paste0("log_cox_models_", data_label, "_", adjustment_set,"_organ_new.csv")))
